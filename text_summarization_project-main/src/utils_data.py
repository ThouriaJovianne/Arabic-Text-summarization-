...
def process_dataframe(df, max_text, max_sum):
    ...
    df['summary'] = df['summary'].apply(lambda x: ' <SUMMARY> ' + x) # do this step in data loader
...
def split_data (df, sr):
	train, validation_set = train_test_split(df,test_size=sr)
	val, test = train_test_split(validation_set,test_size=0.5)
